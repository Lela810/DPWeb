...
const font = {
	calibriLight: {
		data: fs.readFileSync(
			path.join(__dirname, '/assets/fonts/calibri-light.ttf')
		),
		fallback: true,
	},
	calibriBold: {
		data: fs.readFileSync(
			path.join(__dirname, '/assets/fonts/calibri-bold.ttf')
		),
	},
	calibri: {
...
export async function generatePdf() {
	...
	const inputs = [
		{
			anschlag:
				'Die Vorbereitungen für Weihnachten, produzieren viel Abfall. Wir wollen dem Samichlaus zur Seite stehen und ihm helfen den Abfallberg unter Kontrolle zu bringen.\nWeisst du wie wir dem Samichlaus helfen können?',
